...
impl BatteryPack {
    ...
    fn find_y_for_x_month(&mut self, month: &str, x: u32) -> f32 {
        let data = match month {
            "jan" => &self.jan_min_data,
            "jun" => &self.jun_max_data,
            _ => &vec![(0_u32, 0.0)]//panic!("Unsupported month"),
        };
        ...
        for i in start..data.len() {
            ...
            if x0 <= x && x <= x1 {
                ...
                return self.linear_interpolate(x0 as f32, y0, x1 as f32, y1, x as f32);
            }
        }
        0.0
        //panic!("No y for x: {}", x);
    ...
    fn find_x_for_y_month(&mut self, month: &str, y: f32) -> u32 {
        let data = match month {
            "jan" => &self.jan_min_data,
            "jun" => &self.jun_max_data,
            _ => &vec![(0_u32, 0.0)]//panic!("Unsupported month"),
        ...
        for i in start..data.len() {
            ...
            if y0 <= y && y <= y1 {
                self.start_index.insert(month.to_string(), std::cmp::max(1, i - 1));
                return self.linear_interpolate(y0, x0 as f32, y1, x1 as f32, y) as u32;
            }
        }
        0
        //panic!("No x for y: {}", y);
...
impl Battery for BatteryPack {
    ...
    fn charge(&mut self, time: u32, month: u32) {
        ...
        let jan_time = self.find_x_for_y_month("jan", self.energy_wh);
        let jun_time = self.find_x_for_y_month("jun", self.energy_wh);
