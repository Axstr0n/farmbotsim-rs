...
use crate::{
    ...
    units::{
        angle::Angle, angular_velocity::AngularVelocity, duration::Duration, length::Length,
...
impl IsMovement for RombaMovement {
    ...
    fn calculate_new_pose_from_inputs(
        ...
        match inputs {
            MovementInputs::Romba(romba_inputs) => {
                ...
                let max_velocity = if max_velocity > self.max_velocity {
                    self.max_velocity
                } else {
                    max_velocity
                };
                ...
                if omega.to_base_unit().abs() > self.max_angular_velocity.to_base_unit() {
                    let scale =
                        self.max_angular_velocity.to_base_unit() / omega.to_base_unit().abs();
                    m1 *= scale;
                    m2 *= scale;
                ...
                let new_orientation = Angle::degrees(
                    (current_pose.orientation + omega * simulation_step)
                        .to_degrees()
                        .rem_euclid(360.0),
                );

                let direction = Vec2::new(
                    new_orientation.to_radians().cos(),
                    new_orientation.to_radians().sin(),
                );
    ...
    fn calculate_inputs_for_target(
        ...
        let should_turn =
            !desired_orientation.is_close_to(current_pose.orientation, TOLERANCE_ANGLE);
