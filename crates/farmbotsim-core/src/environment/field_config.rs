...
use crate::{
    ...
    environment::{
        farm_entity_module::{
            crop::Crop, farm_entity::FarmEntity, farm_entity_plan::FarmEntityPlan, row::Row,
        },
        obstacle::Obstacle,
    },
...
impl PointFieldConfig {
    ...
    pub fn new(
        ...
        angle: Angle,
        n_lines: u32,
        n_points_per_line: u32,
        line_spacing: Length,
        point_spacing: Length,
        farm_entity_plan_path: String,
    ) -> Self {
        Self {
            id: 0,
...
impl FieldConfig {
        ...
        for config_variant in &self.configs {
            ...
            match config_variant {
                VariantFieldConfig::Line(c) => {
                    let mut pos1 = c.left_top_pos
                        + Vec2::new(
                            (-c.line_spacing / 2.0).to_base_unit(),
                            (-height_offset).to_base_unit(),
                        )
                        .rotate(c.angle);
                    let obstacle_width_2_val = (obstacle_width / 2.0).to_base_unit();
                    for _ in 0..c.n_lines + 1 {
                        let pos2 = pos1
                            + Vec2::new((c.length + 2.0 * height_offset).to_base_unit(), 0.0)
                                .rotate(c.angle + Angle::degrees(90.0));
                ...
                VariantFieldConfig::Point(c) => {
                    ...
                    for _ in 0..c.n_lines + 1 {
                        let pos2 = pos1
                            + Vec2::new((line_length + 2.0 * height_offset).to_base_unit(), 0.0)
                                .rotate(c.angle + Angle::degrees(90.0));
                        let p1 = pos1 + Vec2::new(-obstacle_width_2_val, 0.0).rotate(c.angle);
                        let p2 = pos1 + Vec2::new(obstacle_width_2_val, 0.0).rotate(c.angle);
                        let p3 = pos2 + Vec2::new(obstacle_width_2_val, 0.0).rotate(c.angle);
            ...
            match config_variant {
                VariantFieldConfig::Line(c) => {
                    ...
                    for i in (-1)..=c.n_lines as i32 {
                        let p1 = c.left_top_pos
                            + Vec2::new(ls_val * i as f32, (-offset).to_base_unit())
                                .rotate(c.angle);
                        let p2 = c.left_top_pos
                            + Vec2::new(
                                ls_val * i as f32,
                                c.length.to_base_unit() + offset.to_base_unit(),
                            )
                            .rotate(c.angle);
                        points.push(p1);
                        points.push(p2);
                ...
                VariantFieldConfig::Point(c) => {
                    ...
                    for i in (-1)..=c.n_lines as i32 {
                        ...
                        let p2 = c.left_top_pos
                            + Vec2::new(
                                ls_val * i as f32,
                                line_length.to_base_unit() + offset.to_base_unit(),
                            )
                            .rotate(c.angle);
                        points.push(p1);
                        points.push(p2);
            ...
            match config_variant {
                VariantFieldConfig::Line(c) => {
                    ...
                    for i in 0..c.n_lines {
                        let path = vec![
                            c.left_top_pos + Vec2::new(i as f32 * ls_val, 0.0).rotate(c.angle),
                            c.left_top_pos
                                + Vec2::new(i as f32 * ls_val, c.length.to_base_unit())
                                    .rotate(c.angle),
                        ];
                ...
                VariantFieldConfig::Point(c) => {
                    ...
                    for i in 0..c.n_lines {
                        for j in 0..c.n_points_per_line {
                            let pos = c.left_top_pos
                                + Vec2::new(
                                    c.line_spacing.to_base_unit() * i as f32,
                                    c.point_spacing.to_base_unit() * j as f32,
                                )
                                .rotate(c.angle);
                            let crop =
                                Crop::new(id_counter, field_id, i, pos, farm_entity_plan.clone());
    ...
    pub fn number_of_actions(&self) -> Option<u32> {
        if self.has_cycle_farm_entity_plan() {
            return None;
        }
        ...
        for config in &self.configs {
            match config {
                VariantFieldConfig::Line(data) => {
                    let plan: FarmEntityPlan =
                        load_json_or_panic(data.farm_entity_plan_path.clone());
