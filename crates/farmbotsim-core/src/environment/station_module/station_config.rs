...
use crate::{
    ...
    utilities::utils::line_positions,
};
...
impl StationConfig {
    ...
    pub fn new(
        pose: Pose,
        queue_direction: Angle,
        waiting_offset: Length,
        n_slots: u32,
        slots_pose: Vec<Pose>,
    ) -> Self {
        Self {
    ...
    pub fn update_slots_pose(&mut self) {
        if self.n_slots < self.slots_pose.len() as u32 {
            ...
            let positions = line_positions(
                ...
                Length::meters(0.3).to_base_unit(),
                self.pose.orientation.to_radians(),
            );
            ...
            self.slots_pose
                .extend((0..diff).map(|_| Pose::new(Pos2::ZERO, Angle::degrees(90.0))));
            for (i, slot_pose) in self.slots_pose.iter_mut().enumerate() {
                slot_pose.position = positions[i];
            }
        }
