...
use crate::{
    ...
    tool_module::{
        agent_config_editor_tool::AgentConfigEditorTool, battery_tool::BatteryTool,
        farm_entity_plan_editor_tool::FarmEntityPlanEditorTool,
        field_config_editor_tool::FieldConfigEditorTool, general_help_tool::GeneralHelpTool,
        movement_config_editor_tool::MovementConfigEditorTool, path_tool::PathTool,
        performance_matrix_tool::PerformanceMatrixTool,
        scene_config_editor_tool::SceneConfigEditorTool, simulation_tool::SimulationTool,
        task_manager_config_editor_tool::TaskManagerConfigEditorTool, task_tool::TaskTool,
        tool::Tool,
    },
};
...
impl App {
    ...
    pub fn render(&mut self, ctx: &egui::Context, _frame: &mut eframe::Frame) {
        ...
        egui::SidePanel::right("right_panel")
            ...
            .show(ctx, |ui| {
                ui.label(
                    egui::RichText::new(self.mode.to_string())
                        .strong()
                        .size(20.0),
                );
                ui.separator();
                egui::ScrollArea::vertical().show(ui, |ui| match self.mode {
                    ...
                    AppMode::FarmEntityPlanEditor => {
                        self.farm_entity_plan_editor_tool.render_ui(ui)
                    }
                    ...
                    AppMode::TaskManagerConfigEditor => {
                        self.task_manager_config_editor_tool.render_ui(ui)
                    }
            ...
            match self.mode {
                ...
                | AppMode::SceneConfigEditor => {
                    ...
                    egui::Frame::group(ui.style())
                        ...
                        .show(ui, |ui| match self.mode {
                            AppMode::Simulation => {
                                self.simulation_tool.render_main(ui);
                            }
                            AppMode::Path => {
                                self.path_tool.render_main(ui);
                            }
                            AppMode::Task => {
                                self.task_tool.render_main(ui);
                            }
                            AppMode::FieldConfigEditor => {
                                self.field_config_editor_tool.render_main(ui);
                            }
                            AppMode::SceneConfigEditor => {
                                self.scene_config_editor_tool.render_main(ui);
                            }
                            _ => {}
                        });
                }
                ...
                | AppMode::GeneralHelp => {
                    ...
                    match self.mode {
                        ...
                        AppMode::FarmEntityPlanEditor => {
                            self.farm_entity_plan_editor_tool.render_main(ui)
                        }
                        AppMode::MovementConfigEditor => {
                            self.movement_config_editor_tool.render_main(ui)
                        }
                        ...
                        AppMode::TaskManagerConfigEditor => {
                            self.task_manager_config_editor_tool.render_main(ui)
                        }
