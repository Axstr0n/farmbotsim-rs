...
use crate::{
    ...
    utilities::ui::{folder_select_combo, json_config_combo},
};
...
impl AgentConfigEditorTool {
    ...
    fn ui_agent_config_select(&mut self, ui: &mut Ui) {
            ...
            && new_value != self.current_agent_config_path
        {
            ...
            let agent_config: AgentConfig =
                load_json_or_panic(self.current_agent_config_path.clone());
    ...
    fn ui_battery_select(&mut self, ui: &mut egui::Ui) {
        ...
        if folder_select_combo(ui, "battery_select", &mut new_path, BATTERIES_PATH)
...
impl HasConfigSaving for AgentConfigEditorTool {
    ...
    fn config(&self) -> impl serde::Serialize {
        AgentConfig::new(
            self.current_movement_path.clone(),
            self.current_battery_path.clone(),
...
impl HasHelp for AgentConfigEditorTool {
    ...
    fn render_help_contents(&self, ui: &mut egui::Ui) {
        ...
        ui.label(
            "This is a Agent Config Editor where you can see, change, create, save agent configs.",
        );
        ui.separator();
