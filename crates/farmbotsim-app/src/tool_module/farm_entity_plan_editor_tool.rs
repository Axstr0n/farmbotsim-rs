...
use crate::{
    ...
    utilities::ui::{json_config_combo, value_with_unit_selector_ui},
};
...
impl Tool for FarmEntityPlanEditorTool {
    fn render_main(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ui.label("    plan: [");
            if self.plan.type_ == "point" && ui.button("Add point").clicked() {
                self.plan.schedule.push(FarmEntityAction::default_point());
            }
            if self.plan.type_ == "line" && ui.button("Add line").clicked() {
                self.plan.schedule.push(FarmEntityAction::default_line());
            }
            if ui.button("Add wait").clicked() {
                self.plan.schedule.push(FarmEntityAction::default_wait());
            }
    ...
    fn ui_farm_entity_plan_select(&mut self, ui: &mut Ui) {
            ...
            && new_value != self.current_farm_entity_plan_path
        {
            ...
            let plan: FarmEntityPlan =
                load_json_or_panic(self.current_farm_entity_plan_path.clone());
            self.plan = plan;
        }
    }
    ...
    fn check_cycle(&mut self) {
        if let Some(cycle) = self.plan.cycle {
            if self.plan.schedule.is_empty() {
                self.plan.cycle = None;
            } else if cycle >= self.plan.schedule.len() as u32 {
                self.plan.cycle = Some(self.plan.schedule.len() as u32 - 1)
            }
        }
    }
}
...
impl HasHelp for FarmEntityPlanEditorTool {
    ...
    fn render_help_contents(&self, ui: &mut egui::Ui) {
        ...
        ui.label(
            "This is a Farm Entity Plan Editor where you can see, change, create, save plans.",
        );
        ui.separator();

        ui.label("Type specifies if whole plan is point/stationary or line/moving.");
        ui.label(
            "Cycle parameter specify if after the last action the plan cycles and from which index",
        );
