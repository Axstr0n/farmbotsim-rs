...
use crate::{
    ...
    utilities::files::get_folders_in_folder,
};
        ...
        match &self.selected {
            ...
            Some(selected) => {
                if let Some(battery) = self.battery_map.get(selected) {
                    let jan_max: PlotPoints = battery
                        .jan_max_data
                        .iter()
                    ...
                    let jan_min: PlotPoints = battery
                        .jan_min_data
                        .iter()
                        .map(|(x, y)| [f64::from(*x), f64::from(*y)])
                        .collect::<Vec<_>>()
                    ...
                    let jun_max: PlotPoints = battery
                        .jun_max_data
                        .iter()
                        .map(|(x, y)| [f64::from(*x), f64::from(*y)])
                        .collect::<Vec<_>>()
    ...
    fn render_ui(&mut self, ui: &mut egui::Ui) {
        ...
        if let Some(selected) = &self.selected {
            if let Some(battery) = self.battery_map.get_mut(selected) {
                ...
                let response = ui.add(
                    Slider::new(&mut battery.soc, 0.0..=100.0)
                        .text("SoC [%]")
                        .step_by(1.0),
                );
                if response.changed() {
                ...
                let response = ui.add(
                    Slider::new(&mut self.month, 1..=12)
                        .text("Month")
                        .step_by(1.0),
                );
