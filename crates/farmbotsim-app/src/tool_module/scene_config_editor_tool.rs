...
use crate::{
    rendering::{
        ...
        render::{
            ...
            render_obstacles, render_spawn_area, render_station, render_visibility_graph,
        },
    },
    tool_module::{
        has_camera::HasCamera, has_config_saving::HasConfigSaving, has_help::HasHelp, tool::Tool,
    },
    utilities::ui::json_config_combo,
};
...
impl Tool for SceneConfigEditorTool {
    ...
    fn render_main(&mut self, ui: &mut Ui) {
        ...
        render_spawn_area(
            ui,
            &self.camera,
            &SpawnArea::from_config(self.scene_config.spawn_area_config.clone()),
        );
        ...
        for (i, station_config) in self.scene_config.station_configs.iter().enumerate() {
            render_station(
                ui,
                &self.camera,
                &Station::from_config(0, colors[i], station_config.clone()),
                true,
            );
        }
    ...
    fn render_ui(&mut self, ui: &mut Ui) {
        ...
        ui.label(format!(
            "Left top pos {}",
            self.scene_config.spawn_area_config.left_top_pos.fmt(2)
        ));
        ui.add(
            Slider::new(
            ...
            .text(format!(
                "Angle [{}]",
                self.scene_config.spawn_area_config.angle.unit
            ))
            .step_by(1.0),
        );
        ui.add(
            ...
            .text(format!(
                "Height [{}]",
                self.scene_config.spawn_area_config.height.unit
            ))
            .step_by(0.1),
        );
        ui.add(
            ...
            .text(format!(
                "Width [{}]",
                self.scene_config.spawn_area_config.width.unit
            ))
            .step_by(0.1),
        );
        ...
        ui.label(
            egui::RichText::new(format!(
                "Stations ({}):",
                self.scene_config.station_configs.len()
            ))
            .size(16.0),
        );
        ...
        for (i, station) in self.scene_config.station_configs.iter_mut().enumerate() {
            egui::CollapsingHeader::new({
                ...
                job.append(format!(" {i}").as_str(), 0.0, egui::TextFormat::default());
                job
            })
            .default_open(false)
            .show(ui, |ui| {
                ...
                if ui
                    .add(
                        Slider::new(&mut station.pose.orientation.value, 0.0..=360.0)
                            .text(format!("orientation [{}]", station.pose.orientation.unit))
                            .step_by(1.0),
                    )
                    .changed()
                {
                    station.update_slots_pose();
                }
                ui.add(
                    Slider::new(&mut station.queue_direction.value, 0.0..=360.0)
                        .text(format!(
                            "queue_direction [{}]",
                            station.queue_direction.unit
                        ))
                        .step_by(1.0),
                );
                ui.add(
                    Slider::new(&mut station.waiting_offset.value, 0.1..=3.0)
                        .text("waiting_offset [m]")
                        .step_by(0.1),
                );
                if ui
                    .add(
                        Slider::new(&mut station.n_slots, 1..=3)
                            .text("n_slots")
                            .step_by(1.0),
                    )
                    .changed()
                ...
                for (i, pose) in station.slots_pose.iter_mut().enumerate() {
                    egui::CollapsingHeader::new(format!("slot_pose_{i}"))
                        ...
                        .show(ui, |ui| {
                            ...
                            ui.add(
                                Slider::new(&mut pose.orientation.value, 0.0..=360.0)
...
impl SceneConfigEditorTool {
    ...
    fn handle_dragging(&mut self, ui: &mut Ui) {
        ...
        for station_config in &mut self.scene_config.station_configs {
            let screen_pos = self
                .camera
                .scene_to_screen_pos(station_config.pose.position);
            pts.push(screen_pos);
        }
        let spawn_pos = self
            .camera
            .scene_to_screen_pos(self.scene_config.spawn_area_config.left_top_pos);
        pts.push(spawn_pos);
        ...
        for (i, station_config) in &mut self.scene_config.station_configs.iter_mut().enumerate() {
            ...
            let response = ui.interact(
                rect,
                ui.make_persistent_id(format!("station_drag_{i}")),
        ...
        let response = ui.interact(
            rect,
            ui.make_persistent_id("spawn_drag"),
            egui::Sense::click_and_drag(),
        );
