...
impl Tool for PerformanceMatrixTool {
    ...
    fn render_ui(&mut self, ui: &mut egui::Ui) {
        ...
        ui.horizontal(|ui| {
            ui.label("n_episodes: ");
            ui.add(
                egui::DragValue::new(&mut self.n_episodes)
                    .speed(10)
                    .range(10..=10000),
            );
        });
        ...
        ui.horizontal(|ui| {
            ...
            ui.label(format!(
                "{} {} |",
                self.datetime_config.date, self.datetime_config.time
            ));
        ...
        // env configs
        ui.label(
            egui::RichText::new(format!("Env configs ({}):", self.env_configs.len())).size(16.0),
        );
        ui.horizontal(|ui| {
            ...
            egui::CollapsingHeader::new(format!("Config {i}"))
                ...
                .show(ui, |ui| {
                    ...
                    ui.horizontal(|ui| {
                        ui.label("n_agents: ");
                        ui.add(
                            egui::DragValue::new(&mut config.n_agents)
                                .speed(1)
                                .range(1..=10),
                        );
                    });
        ...
        ui.horizontal(|ui| {
            ...
            egui::ComboBox::from_id_salt("Termination Condition")
                ...
                .show_ui(ui, |ui| {
                    if !Self::has_cycle_plan(self.scene_config_path.clone())
                        && ui
                            .selectable_label(
                    ...
                    if ui
                        .selectable_label(selected_kind == "EnvDuration", "EnvDuration")
                        .clicked()
                    {
                        self.termination_condition =
                            TerminationCondition::EnvDuration(Duration::days(1.0)); // default
            ...
            match &mut self.termination_condition {
                TerminationCondition::NumberCompletedTasks(val) => {
                    ...
                    let scene_config: SceneConfig =
                        load_json_or_panic(self.scene_config_path.clone());
        ...
        if self.running {
            ui.label(
                egui::RichText::new("Make sure you set tps/fps ratio high in settings")
                    .color(egui::Color32::RED),
            );
            ui.label("Evaluating...");
            let progress = (self.env_index as u32 * self.n_episodes + self.env_episode) as f32
                / (self.env_configs.len() as u32 * self.n_episodes) as f32;
...
impl PerformanceMatrixTool {
    ...
    fn ui_result_select(&mut self, ui: &mut egui::Ui) {
        ...
        let mut selected_path = self
            .current_pm_path
            .clone()
            .unwrap_or("Select file...".to_string());
    ...
    fn finalize_result(&mut self) {
        ...
        let env_results: Vec<EnvResult> = self
            .env_configs
            .iter()
            .enumerate()
            .map(|(i, config)| {
